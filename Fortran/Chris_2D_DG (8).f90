...
program wave_equation
  ...
  direct_solver=.true. ! use directi solver? 
  ! max_face_list_no = 4
...
!AMIN l1, l2, l3, l4 aren't used anywhere
  ...
  allocate(face_sn(sngi,nloc,nface), face_sn2(sngi,nloc,n_s_list_no), face_snlx(sngi,sndim,nloc,nface), face_sweigh(sngi,nface))
  allocate(x_all(ndim,nloc,totele), x_loc(ndim,nloc), jac(ndim*ndim))
  allocate(mass_ele(nloc,nloc), mass_ele_inv(nloc,nloc), rhs_loc(nloc), ml_ele(nloc), rhs_jac(nloc), mass_t_new(nloc) )
...
! r_got_boundary=0.0 not on boundary
      ...
      ! inverse of the mass matric (nloc,nloc)
         ...
         do jac_its=1,njac_its ! jacobi iterations...
            do iloc=1,nloc
               mass_t_new(iloc)= sum( mass_ele(iloc,:) * t_new(:,ele) ) 
            end do
            do iloc=1,nloc
               t_new(iloc,ele)=  (ml_ele(iloc)*t_new(iloc,ele) - mass_t_new(iloc) + rhs_jac(iloc) ) / ml_ele(iloc)
            end do
         end do
